    ...
    useEffect(() => {
        ...
        const fetchWorkers = async () => {
          try {
            ...
            const workersWithTypes = allWorkers.map(worker => ({ ...worker, type: 'worker' })); 
        ...
        >
          {filteredResults.map((result, index) => (
            <Box
              ...
              onClick={() => handleResultClick(result._id.$oid, result.name, result.type)}
            >
              <Avatar sx={{ bgcolor: result.type === 'job' ? "secondary.main" : "primary.main", mr: 2 }}>
